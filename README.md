...
```bash
multi-step$ sam build --use-container
```
...
```bash
multi-step$ sam logs -n StockCheckerFunction --stack-name multi-step --tail
```
...
```bash
multi-step$ pip install -r tests/requirements.txt --user
# unit test
multi-step$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
multi-step$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name multi-step
```
